...
CREATE TABLE `zm_goods`.`goods_base` (
  ...
  `excise_tax` DECIMAL(4,2) NULL COMMENT '消费税',
  `increment_tax` DECIMAL(4,2) NULL COMMENT '增值税',
  `tariff` DECIMAL(4,2) NULL COMMENT '关税',
...
CREATE TABLE `zm_goods`.`goods_stock` (
  ...
  `sjqty` int UNSIGNED NULL COMMENT '上架数量', 
