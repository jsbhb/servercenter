CREATE TABLE IF NOT EXISTS `zm_goods`.`component` (
  ...
  `data` VARCHAR(1000) NULL COMMENT '数据json串',
...
CREATE TABLE IF NOT EXISTS `zm_goods`.`page` (
  ...
  `type` TINYINT(3) UNSIGNED NULL COMMENT '类型:1首页,2活动',
  `status` TINYINT(3) UNSIGNED NULL DEFAULT 0 COMMENT '状态:0编辑中,1草稿,2发布',
  `client` TINYINT(3) UNSIGNED NULL COMMENT '终端:0:pc,1:H5',
  `grade_id` INT UNSIGNED NOT NULL COMMENT '分级ID',
  `fpath` VARCHAR(100) NULL COMMENT '前端路径',
...
CREATE TABLE IF NOT EXISTS `zm_goods`.`component_page` (
  ...
  `tag_path` VARCHAR(200) NULL COMMENT '图标地址',
  `title` VARCHAR(45) NULL COMMENT '名称',
  `enname` VARCHAR(45) NULL COMMENT '英文名称',
  `href` VARCHAR(200) NULL COMMENT '链接',
  `backpic` VARCHAR(200) NULL COMMENT '背景图片地址',
  `description` VARCHAR(45) NULL COMMENT '描述',
