...
CREATE TABLE `zm_order`.`order_base` (
  ...
  `create_type` TINYINT UNSIGNED NOT NULL COMMENT '0:普通订单；1：活动订单；2：优惠券订单；3：活动+优惠券;4:预售订单;5:对接订单',
  ...
  `push_user_id` INT UNSIGNED NULL  COMMENT '如果有推手ID存在这里',
  ...
  INDEX `idx_push_user_id` (`push_user_id` ASC),
