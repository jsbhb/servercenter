...
public class LogAopUtil {
	...
	// 异常类日志
	@Pointcut("execution(* com.zm.order.bussiness.service.impl.*.*(..))")
	...
	public void afterThrowException(JoinPoint jp, Throwable e) {
		...
		ExceptionLog exceptionLog = new ExceptionLog(ServerLogEnum.ORDER_CENTER.getServerId(),
				ServerLogEnum.ORDER_CENTER.getServerName(), methodName, sb.toString());
		try {
	...
	// 开放接口调用日志
	@Pointcut("execution(* com.zm.order.bussiness.service.impl.OrderOpenInterfaceServiceImpl.*(..))")
...
//		String callIp = request.getRemoteAddr();
	...
	// 自定义注解日志
	@Pointcut("@annotation(com.zm.order.annotation.Log)")
		...
		try {
			if (arguments != null && arguments.length > 0) {
				for (Object obj : arguments) {
					if(obj.getClass().getName().contains("com.zm.order")){
...
//		String callIp = request.getRemoteAddr();
		...
		LogInfo logInfo = new LogInfo(parameter, source, type, ip, methodName, content,
				ServerLogEnum.ORDER_CENTER.getServerId());
		try {
