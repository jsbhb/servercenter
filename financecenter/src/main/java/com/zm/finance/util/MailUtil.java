...
public class MailUtil {
	...
	public void doSendHtmlEmail(String subject, String sendHtml, String[] receiveUser, File attachment) {
		try {
			...
			for (int i = 0; i < receiveUser.length; i++) {
				to = new InternetAddress(receiveUser[i]);
				// 收件人
				if (i == 0) {
			...
			LogUtil.writeLog("邮件发送成功");
		} catch (Exception e) {
			LogUtil.writeErrorLog("邮件发送出错", e);
			e.printStackTrace();
		} finally {
			if (transport != null) {
				try {
	...
	// File affix = new File("C:\\Users\\user\\Desktop\\正正\\入库.rar");
	// se.doSendHtmlEmail("邮件主题", "邮件内容", "1301518675@qq.com", affix);//
	// }
